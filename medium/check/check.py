...
def check(king, queen):
    ...
    k_row, k_col = 8 - int(king[1]), letter_to_num[king[0]]
    q_row, q_col = 8 - int(queen[1]), letter_to_num[queen[0]]

    if k_row == q_row or k_col == q_col:
        return True

    if abs(k_row - q_row) == abs(k_col - q_col):
        return True
