...
def earliest_arrival(jump_distance, stones):
    ...
    # starting index of the monkey
    pos = -1
    landings = []

    # iterate over each time, return first successful crossing
    while pos + jump_distance < len(stones):
        
        # where monkey can jump
        possible_landings = stones[pos + 1:pos + jump_distance + 1]
        # soonest place monkey can jump
        soonest_landing = min(possible_landings)
        # track where the monkey jumped
