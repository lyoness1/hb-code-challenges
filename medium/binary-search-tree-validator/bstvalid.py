...
class Node:
    ...
    def is_valid(self):
        ...
        def _valid(self, ceiling, floor):

            # base case: not a node
            if not self:
                return True

            # base case: left is too big
            if ceiling and ceiling < self.data:
                return False

            # base case: right is too small
            if floor and floor > self.data:
                return False

            # check whole left tree with ceiling as self.data
            if not _valid(self.left, self.data, floor):
                return False

            # check whole right tree with floor as self.data
            if not _valid(self.right, ceiling, self.data):
                return False

            # base case: nothing failed --> win!
            return True

        # start here: no ceiling or floor because curr node not yet checked
        return _valid(self, None, None)
